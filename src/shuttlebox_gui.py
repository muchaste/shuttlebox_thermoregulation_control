...
class TempControlController(SerialDevice):
    ...
    def __init__(self, port: str):
        ...
        # Track desired vs actual states for stability
        self.desired_states = {
            "HEAT": False,
            "COOL": False,
            "BHEAT": False,
        ...
        self.command_timeout = 2.0  # 2 seconds timeout for relay commands
    
    def set_heating(self, enable: bool):
        """Set heating relay state with echo-based verification"""
        if self.desired_states["HEAT"] != enable:
            self.desired_states["HEAT"] = enable
            command = "HEAT_ON" if enable else "HEAT_OFF"
            self.send_command(command)
            self.last_command_time["HEAT"] = time.time()
    
    def set_cooling(self, enable: bool):
        """Set cooling relay state with echo-based verification"""
        if self.desired_states["COOL"] != enable:
            self.desired_states["COOL"] = enable
            command = "COOL_ON" if enable else "COOL_OFF"
            self.send_command(command)
            self.last_command_time["COOL"] = time.time()
    
    def set_buffer_heating(self, enable: bool):
        """Set buffer heating relay state with echo-based verification"""
        if self.desired_states["BHEAT"] != enable:
            self.desired_states["BHEAT"] = enable
            command = "BUFFER_HEAT_ON" if enable else "BUFFER_HEAT_OFF"
            self.send_command(command)
            self.last_command_time["BHEAT"] = time.time()
    
    def set_buffer_cooling(self, enable: bool):
        """Set buffer cooling relay state with echo-based verification"""
        if self.desired_states["BCOOL"] != enable:
            self.desired_states["BCOOL"] = enable
            command = "BUFFER_COOL_ON" if enable else "BUFFER_COOL_OFF"
            self.send_command(command)
            self.last_command_time["BCOOL"] = time.time()
    
    def emergency_stop(self):
        """Emergency stop all relays"""
        self.send_command("ALL_OFF")
        # Reset all desired states
        for relay in self.desired_states:
            self.desired_states[relay] = False
            self.last_command_time[relay] = time.time()
    
    def get_status(self):
        """Get current relay status and process any new messages"""
        while not self.message_queue.empty():
            timestamp, message = self.message_queue.get()
    ...
    def get_relay_sync_status(self):
        """Check if actual relay states match desired states"""
        sync_status = {}
        ...
        for relay in self.relay_states:
            ...
            sync_status[relay] = {
                'actual': actual,
                'desired': desired,
                'synced': actual == desired,
                'timeout': time_since_cmd > self.command_timeout,
    ...
    def retry_failed_commands(self):
        """Retry commands for relays that aren't in sync"""
        sync_status = self.get_relay_sync_status()
        
        for relay, status in sync_status.items():
            if not status['synced'] and status['timeout']:
                # Retry the command
                if relay == "HEAT":
                    self.set_heating(status['desired'])
                elif relay == "COOL":
                    self.set_cooling(status['desired'])
                elif relay == "BHEAT":
                    self.set_buffer_heating(status['desired'])
                elif relay == "BCOOL":
                    self.set_buffer_cooling(status['desired'])
                
                print(f"Retrying {relay} relay command (desired: {status['desired']}, actual: {status['actual']})")
    
    def keep_alive(self):
        """Send ping to prevent Arduino command timeout"""
        if self.connected:
        ...
        if message.startswith("RELAY_"):
            ...
            if len(parts) == 2:
                ...
                if relay_type in self.relay_states:
                    ...
                    # Log successful relay state changes
                    if relay_type in self.desired_states and self.relay_states[relay_type] == self.desired_states[relay_type]:
                        print(f"Relay {relay_type} successfully set to {state}")
        elif message.startswith("ERROR:"):
            print(f"Arduino error: {message}")
        elif message == "RESET_COMPLETE":
            # Reset all states after Arduino reset
            for relay in self.relay_states:
...
class ShuttleboxGUI:
    ...
    def create_left_panel(self, parent):
        ...
        ttk.Label(arduino_frame, text="Temp Control COM Port:").grid(row=current_row, column=0, sticky=tk.W, pady=2)
        ...
        self.temp_com_combo.grid(row=current_row, column=1, padx=(10, 0))
    ...
    def _update_auto_reconnect_setting(self):
        """Update auto-reconnect settings from UI"""
        self.tc08_auto_reconnect_enabled = self.auto_reconnect_var.get()
        try:
            threshold = int(self.failure_threshold_var.get())
            if threshold > 0:
                self.tc08_failure_threshold = threshold
        except ValueError:
            self.failure_threshold_var.set(str(self.tc08_failure_threshold))
    ...
    def _attempt_tc08_auto_reconnect(self):
        """Attempt automatic TC-08 reconnection after failures"""
        if not self.tc08_auto_reconnect_enabled:
            print("TC-08 auto-reconnection is disabled")
            return False
        
        if self.tc08_reconnection_in_progress:
            print("TC-08 reconnection already in progress")
            return False
        
        # Check cooldown period
        if self.tc08_last_reconnection_attempt:
            elapsed = (datetime.now() - self.tc08_last_reconnection_attempt).total_seconds()
            if elapsed < self.tc08_reconnection_cooldown:
                print(f"TC-08 reconnection cooldown: {self.tc08_reconnection_cooldown - elapsed:.0f}s remaining")
                return False
        
        print("="*60)
        print("🔄 Attempting TC-08 auto-reconnection...")
        print("="*60)
        
        self.tc08_reconnection_in_progress = True
        self.tc08_last_reconnection_attempt = datetime.now()
        ...
        try:
            ...
            if self.temp_control_controller:
                ...
                print(f"Saved relay states: {relay_states_before}")
            
            # Save control mode
            was_static_active = self.static_control_active
            was_dynamic_active = self.dynamic_control_active
            
            # Disconnect TC-08
            print("Step 1: Disconnecting TC-08...")
            ...
            print("Step 2: Waiting for USB stabilization (5 seconds)...")
            time.sleep(5)
            
            # Attempt reconnection
            print("Step 3: Reconnecting TC-08...")
            success = False
            try:
                # Use the same configuration
                config = TC08Config(
                    channels=self.config.tc08_channels,
                    sample_interval_ms=int(self.tc08_interval_var.get()),
                    thermocouple_type='K'
                )
                ...
                if self.tc08_controller.connect():
                    ...
                    print("❌ Failed to reconnect TC-08")
            except Exception as e:
                print(f"❌ Reconnection error: {e}")
            
            # Restore relay states (important for ongoing experiments)
            if success and relay_states_before and self.temp_control_controller:
                print("Step 4: Restoring relay states...")
                for relay, state in relay_states_before.items():
                    if relay == "HEAT":
                        self.temp_control_controller.set_heating(state)
                    elif relay == "COOL":
                        self.temp_control_controller.set_cooling(state)
                    elif relay == "BHEAT":
                        self.temp_control_controller.set_buffer_heating(state)
                    elif relay == "BCOOL":
                        self.temp_control_controller.set_buffer_cooling(state)
                print(f"Restored relay states: {relay_states_before}")
            
            # Log reconnection attempt
            if self.tc08_controller and self.tc08_controller.health_monitor:
                self.tc08_controller.health_monitor.log_connection_event(
                    "auto_reconnection",
                    success,
                    {
                        "relay_states_preserved": relay_states_before is not None,
                        "control_mode_active": was_static_active or was_dynamic_active
                    }
                )
            ...
            print(f"❌ Critical error during auto-reconnection: {e}")
            return False
        finally:
            self.tc08_reconnection_in_progress = False
    ...
    def start_recording(self):
        ...
        try:
            with open(metadata_filename, 'w') as f:
                ...
                # Use actual connected ports instead of config defaults
                ir_port = self.ir_com_var.get() if self.ir_sensor_controller else "Not connected"
                temp_port = self.temp_com_var.get() if self.temp_control_controller else "Not connected"
                f.write(f"IR Sensor Port: {ir_port}\n")
                f.write(f"Temperature Control Port: {temp_port}\n\n")
        ...
        try:
            ...
            if self.plotting_active and self.tc08_controller:
                try:
                    ...
                    if not self._attempt_tc08_auto_reconnect():
                        self.plotting_active = False
                except Exception as e:
